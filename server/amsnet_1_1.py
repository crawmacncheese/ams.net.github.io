...
if len(sys.argv) < 2:
        print("Usage: python amsnet_1_1.py <user_uuid>")
        sys.exit(1)
user_uuid = sys.argv[1]
imgnumber = sys.argv[2]
...
page_fns = natsorted(glob.glob(f'components/{user_uuid}/{imgnumber}.png'))
for i, page_fn in enumerate(page_fns):
  ...
  page_img.save(f'{page_img_path}/{imgnumber}.jpg')
  np_save(page_bin, f'{page_bin_path}/{imgnumber}.npy')
...
arial_font = ImageFont.truetype('arial.ttf', 20)
...
def crop_part(page_fn, top, left, bottom, right, rotate=True, save_fn=''):
  ...
  part_bin_fns = [f'{part_bin_path}/{part_type}_{angle}_{part_instance}.npy' for angle in part_angles]
...
def part_match_all(page_fn, visualize=True):
  page_bin = cache_selection(page_fn)
  bounds = {}
  part_fns = glob.glob(f'{root_path}/part_bin/*.npy')
  print(part_fns)
  print(page_fn)
...
for i in data['rectangle']:
  print(i)
  # print(page_bin_path)
  crop_part(f'{page_bin_path}/{imgnumber}.npy', i["y1"], i["x1"], i["y2"], i["x2"], save_fn=i["label"])
...
try:
  ...
  bbox_json_path = f'{export_dir}/{page_id}/{page_id}_bbox.json'
  cir_file_path = f'{export_dir}/{page_id}/{page_id}.cir'
  if not os.path.exists(bbox_json_path):
    bboxdata = json.dumps({}, indent= 2)
    with open(bbox_json_path, 'w', encoding='utf-8') as f:
          f.write(bboxdata)
  if not os.path.exists(cir_file_path):
    cirdata = "*SPICE Netlist\n\n"
    with open(cir_file_path, 'w', encoding='utf-8') as f:
          f.write(cirdata)
